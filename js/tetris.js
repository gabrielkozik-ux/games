...
window.getPlayableHeight = function() {
    // FIX: Agresivnější ořez pro jistotu
    // Odečteme prostor pro horní lištu (cca 60px) a spodní ovládání/margin (cca 100px)
    const headerAndFooterSpace = 160; 
    const availableHeight = window.innerHeight - headerAndFooterSpace;
    
    // Na mobilech musíme být ještě opatrnější kvůli URL baru
    const mobileSafety = window.innerWidth < 768 ? 40 : 0;
    
    return Math.max(300, availableHeight - mobileSafety); // Minimálně 300px výška
};
